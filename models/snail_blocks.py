...
class AttentionBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        #byte depricated mask = torch.ByteTensor(mask).cuda()
        ...
        #depricated tanh and sigmoid temp = F.softmax(temp / self.sqrt_key_size, dim=1) # shape: (N, T, T), broadcasting over any slice [:, x, :], each row of the matrix
